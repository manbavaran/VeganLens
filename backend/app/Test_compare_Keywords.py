...
################################################
# --- 교차오염 관련 문구 식별을 위한 키워드 (추가) ---
CAUTION_STATEMENT_KEYWORDS = [
    "같은 제조시설", "교차오염", "혼입가능", "알레르기 유발물질",
    "사용된 제품과 같은 제조시설", "제조하고 있습니다"
]
#####################################################
...
def process_image_with_google_vision_only(response, user_type: str = "Vegan"):
    ...
    try:
        ...
        if final_refined_text:
            # --- 비건 금지 성분 확인 ---
            test_ban_list = ban_List(user_type=user_type)
            found_forbidden = check_forbidden_ingredients(final_refined_text, test_ban_list)
            ...
            if found_caution:
                print(f"\n⚠️ 주의: 교차오염 가능 성분 발견! ({len(found_caution)}개)")
