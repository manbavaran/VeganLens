...
def choice(image, debug=True, base_filename=None, version = 1, who='BHY'):
    if (who == 'BHY' & version == 1):
        return extract_text(image, debug=True, base_filename=base_filename, version=1)
    elif (who == 'BHY' & version == 2):
        return extract_text(image, debug=True, base_filename=base_filename, version=2)
